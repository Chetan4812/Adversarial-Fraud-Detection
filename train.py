...
def run_training(transaction_path: str, identity_path: str, artifact_dir: str):
    """
    ...
    # ---------------------------------------------------------------------------
    if not Path(transaction_path).exists() or not Path(identity_path).exists():
        raise FileNotFoundError(
            f"Could not find data files: {transaction_path} or {identity_path}. "
            "Please ensure they are in the specified directory."
        )

    merged = load_and_merge(transaction_path, identity_path)
    ...
    if 1 in val_counts and 0 in val_counts:
        scale_pos_weight = val_counts[0] / val_counts[1]
    else:
        scale_pos_weight = 1.0
        
    class_weight_ratio = scale_pos_weight
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Adversarial Fraud Detection Training Pipeline")
    parser.add_argument("--transaction", default="train_transaction.csv", help="Path to transaction CSV")
    parser.add_argument("--identity", default="train_identity.csv", help="Path to identity CSV")
    parser.add_argument("--artifacts", default="./artifacts", help="Directory to save artifacts")
